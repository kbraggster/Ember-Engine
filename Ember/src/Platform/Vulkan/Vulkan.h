...
inline const char* VKResultToString(VkResult result)
{
    ...
    EM_CORE_ASSERT(false, "not implemented");
    return nullptr;
}
...
inline void VulkanCheckResult(VkResult result)
{
    ...
    {
        EM_CORE_ERROR("VkResult is '{0}'", Utils::VKResultToString(result));
        ...
        EM_CORE_ASSERT(result == VK_SUCCESS, "");
    }
}
...
inline void VulkanCheckResult(VkResult result, const char* file, int line)
{
    if (result != VK_SUCCESS)
    {
        EM_CORE_ERROR("VkResult is '{0}' in {1}:{2}", Utils::VKResultToString(result), file, line);
        if (result == VK_ERROR_DEVICE_LOST)
        {
            using namespace std::chrono_literals;
            std::this_thread::sleep_for(3s);
...
#define VK_CHECK_RESULT(f)                                                                                             \
    {                                                                                                                  \
        VkResult res = (f);                                                                                            \
        ::Ember::Utils::VulkanCheckResult(res, __FILE__, __LINE__);                                                    \
    }
